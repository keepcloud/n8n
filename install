...
if [[ ! -e /var/www/n8n-install.log ]]; then
    touch /var/www/n8n-install.log
fi

logfile=/var/www/n8n-install.log

if [[ ! -e /var/www/.npm ]]; then
    mkdir -p /var/www/.npm
fi
...
if (whiptail --backtitle "n8n Easy Installer" --title "Continue with install?" --yesno "Do you wish to continue with the installation?" 8 78); then
    ...
    npm install @fortawesome/fontawesome-svg-core@1.x
    ...
    npm config set user root install cpu-features
